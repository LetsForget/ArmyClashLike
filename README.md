...
* Использование **FSM** позволяет четко отделить логику состояний и избежать запутанных условий переходов.
* Применение **Zenject** упрощает тестирование, замену зависимостей и улучшает модульность.
* Разделение ответственности между состояниями и контроллерами делает систему гибкой для расширения — например, легко добавить новые типы уровней или состояний.
* Логика UI и геймплея отделена от бизнес-логики FSM, что повышает читаемость и поддерживаемость проекта.

## Возможности для расширения

* Добавление новых игровых состояний (например, пауза, результаты боя).
* Внедрение сохранений прогресса между сессиями.
* Расширение системы контроллеров для разных типов юнитов или AI.
