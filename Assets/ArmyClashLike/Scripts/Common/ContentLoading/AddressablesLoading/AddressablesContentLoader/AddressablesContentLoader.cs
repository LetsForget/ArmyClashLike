...
namespace ContentLoading
{
    public class AddressablesContentLoader : IContentLoader
    {
        ...
        {
            var cached = await CheckCache<T>(assetRef);
        ...
        private async UniTask<CacheCheckResult<T>> CheckCache<T>(AssetReference assetRef)
        {
            if (assetCache.TryGetValue(assetRef, out var loadingHandle))
            {
                ...
                return new CacheCheckResult<T>((T)loadingHandle.Result, true);
            }
        ...
        public struct CacheCheckResult<T>
        {
            public T result;
